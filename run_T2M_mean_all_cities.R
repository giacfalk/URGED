...
# run GVI -> T2M mean analysis for all PROVIDE cities
# creates outputs:
#   - summary_city_lcz_month_T2Mmean_raw_gvi.csv
#   - summary_city_lcz_month_T2Mmean_raw_gvi_significant.csv
#   - summary_city_lcz_month_T2Mmean_raw_gvi_with_se.csv
...
# estimation script
source("estimate_gvi_coefs_T2M_mean_panel_multicities.R")

# MAIN

cat("\n============================================================\n")
cat("T2M mean ANALYSIS - RAW GVI SCALE (0-100)\n")
cat("============================================================\n")
cat("Coefficient interpretation: °C per 1 unit GVI (0-100 scale)\n\n")

# getting list of cities with T2M data
# checking which cities have complete T2M mean data (10 years)
city_dirs <- list.dirs(DATA_DIR, recursive = FALSE, full.names = FALSE)
cities_with_T2M <- c()

for (city in city_dirs) {
  files <- list.files(file.path(DATA_DIR, city), pattern = "T2M_year_daily_mean_20")
  if (length(files) >= 10) {
...
cat("Cities with complete T2M mean data (10 years):", length(cities_with_T2M), "\n")

if (length(cities_with_T2M) == 0) {
  cat("\nNo cities with complete T2M data found.\n")
  cat("Run download_T2M_mean_all_cities.R first to download the data.\n")
...
for (i in seq_along(cities_with_T2M)) {
  ...
  tryCatch({
    res <- estimate_T2M_panel(city, "mean", YEARS)
...
write.csv(output_with_se,
          file.path(OUTPUT_DIR, "summary_city_lcz_month_T2Mmean_raw_gvi_with_se.csv"),
          row.names = FALSE)
cat("  Saved: summary_city_lcz_month_T2Mmean_raw_gvi_with_se.csv\n")

# simple format (raw gvi)
output_simple <- combined_df %>%
  select(city, lcz, month, coef)
write.csv(output_simple,
          file.path(OUTPUT_DIR, "summary_city_lcz_month_T2Mmean_raw_gvi.csv"),
          row.names = FALSE)
cat("  Saved: summary_city_lcz_month_T2Mmean_raw_gvi.csv\n")

# saving also to root directory 
write.csv(output_simple, "summary_city_lcz_month_T2Mmean_raw_gvi.csv", row.names = FALSE)
cat("  Saved: summary_city_lcz_month_T2Mmean_raw_gvi.csv (root)\n")
...
write.csv(output_significant,
          file.path(OUTPUT_DIR, "summary_city_lcz_month_T2Mmean_raw_gvi_significant.csv"),
          row.names = FALSE)
cat("  Saved: summary_city_lcz_month_T2Mmean_raw_gvi_significant.csv\n")

# save to root directory
write.csv(output_significant, "summary_city_lcz_month_T2Mmean_raw_gvi_significant.csv", row.names = FALSE)
cat("  Saved: summary_city_lcz_month_T2Mmean_raw_gvi_significant.csv (root)\n")
...
write.csv(output_sig_cooling,
          file.path(OUTPUT_DIR, "summary_city_lcz_month_T2Mmean_raw_gvi_significant_cooling.csv"),
          row.names = FALSE)
cat("  Saved: summary_city_lcz_month_T2Mmean_raw_gvi_significant_cooling.csv\n")
...
cat("\n============================================================\n")
cat("DONE - T2M mean ANALYSIS COMPLETE\n")
cat("============================================================\n")
