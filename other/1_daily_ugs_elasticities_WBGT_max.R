...
for(min_max_avg_sel in c("max")){
  ...
  for(city_n in which(ff_c %in% out_ndvi_m$city_provide)){
    ...
    names(WBGT_out) <- paste0("mean.WBGT_", names(WBGT_out))
    
    tas_out_m1_m <- WBGT_out
    ...
    tapply(out_ndvi_m_f$`mean.WBGT_2008-01-01`, out_ndvi_m_f$lcz, summary)
    
    ###
    
    out_ndvi_m_monthly = dplyr::select(out_ndvi_m_f, x, y, out_b, Cls, build_h , build_v , pop_dens , water , elevation , city , lcz, contains("mean.WBGT_"))
    ...
    out_ndvi_m_monthly$variable <- unlist(str_split(out_ndvi_m_monthly$variable, pattern = "_"))[c(FALSE, TRUE)]
    out_ndvi_m_monthly$day <- lubridate::day(out_ndvi_m_monthly$variable)
    out_ndvi_m_monthly$month <- lubridate::month(out_ndvi_m_monthly$variable)
    out_ndvi_m_monthly$year <- lubridate::year(out_ndvi_m_monthly$variable)
    ...
    m1 <- lapply(unique(out_ndvi_m_monthly$lcz), function(lcz_sel){
     ...
     if(length(unique(out_ndvi_m_monthly %>% filter(lcz==lcz_sel) %>% pull(out_b)))>1){
      ...
      m1 <- feols(value ~  out_b:as.factor(month) + build_h + build_v + elevation + water | month + year, data=out_ndvi_m_monthly %>% filter(lcz==lcz_sel))
    ...
    m1$variable <- gsub("out\\_b\\:as\\.factor\\(month\\)", "", m1$variable)
    
    m1$city <- ff_c[city_n]
    
    saveRDS(m1, paste0(stub0, "results/elasticities/daily/elasticities_ugs_wbgt_daily_", min_max_avg, "_", ff_c[city_n], ".rds"))
