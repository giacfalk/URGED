...
# run GVI -> WBGT min analysis for all PROVIDE cities
# creates outputs:
#   - summary_city_lcz_month_WBGTmin_raw_gvi.csv
#   - summary_city_lcz_month_WBGTmin_raw_gvi_significant.csv
#   - summary_city_lcz_month_WBGTmin_raw_gvi_with_se.csv
...
# estimation script
source("estimate_gvi_coefs_WBGT_min_panel_multicities.R")

# MAIN

cat("\n============================================================\n")
cat("WBGT min ANALYSIS - RAW GVI SCALE (0-100)\n")
cat("============================================================\n")
cat("Coefficient interpretation: °C per 1 unit GVI (0-100 scale)\n\n")

# getting list of cities with WBGT data
# checking which cities have complete WBGT min data (10 years)
city_dirs <- list.dirs(DATA_DIR, recursive = FALSE, full.names = FALSE)
cities_with_wbgt <- c()

for (city in city_dirs) {
  files <- list.files(file.path(DATA_DIR, city), pattern = "WBGT_year_daily_min_20")
  if (length(files) >= 10) {
...
cat("Cities with complete WBGT min data (10 years):", length(cities_with_wbgt), "\n")

if (length(cities_with_wbgt) == 0) {
  cat("\nNo cities with complete WBGT data found.\n")
  cat("Run download_WBGT__all_cities.R first to download the data.\n")
...
for (i in seq_along(cities_with_wbgt)) {
  ...
  tryCatch({
    res <- estimate_wbgt_panel(city, "min", YEARS)
...
write.csv(output_with_se,
          file.path(OUTPUT_DIR, "summary_city_lcz_month_WBGTmin_raw_gvi_with_se.csv"),
          row.names = FALSE)
cat("  Saved: summary_city_lcz_month_WBGTmin_raw_gvi_with_se.csv\n")

# simple format (raw gvi)
output_simple <- combined_df %>%
  select(city, lcz, month, coef)
write.csv(output_simple,
          file.path(OUTPUT_DIR, "summary_city_lcz_month_WBGTmin_raw_gvi.csv"),
          row.names = FALSE)
cat("  Saved: summary_city_lcz_month_WBGTmin_raw_gvi.csv\n")

# saving also to root directory 
write.csv(output_simple, "summary_city_lcz_month_WBGTmin_raw_gvi.csv", row.names = FALSE)
cat("  Saved: summary_city_lcz_month_WBGTmin_raw_gvi.csv (root)\n")
...
write.csv(output_significant,
          file.path(OUTPUT_DIR, "summary_city_lcz_month_WBGTmin_raw_gvi_significant.csv"),
          row.names = FALSE)
cat("  Saved: summary_city_lcz_month_WBGTmin_raw_gvi_significant.csv\n")

# save to root directory
write.csv(output_significant, "summary_city_lcz_month_WBGTmin_raw_gvi_significant.csv", row.names = FALSE)
cat("  Saved: summary_city_lcz_month_WBGTmin_raw_gvi_significant.csv (root)\n")
...
write.csv(output_sig_cooling,
          file.path(OUTPUT_DIR, "summary_city_lcz_month_WBGTmin_raw_gvi_significant_cooling.csv"),
          row.names = FALSE)
cat("  Saved: summary_city_lcz_month_WBGTmin_raw_gvi_significant_cooling.csv\n")
...
cat("\n============================================================\n")
cat("DONE - WBGT  ANALYSIS COMPLETE\n")
cat("============================================================\n")
