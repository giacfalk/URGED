...
for(min_max_avg_sel in c("mean", "min", "max")){
  ...
  for(city_n in which(ff_c %in% out_ndvi_m$city_provide)){
    ...
    write.csv(out_ndvi_m_monthly_stats, paste0("results/URBCLIM_historical/t_/t_stats_", min_max_avg, "_", ff_c[city_n], ".csv"))
    
    #
    
    write.csv(out_ndvi_m_monthly %>% dplyr::select(city, lcz, variable, value), paste0("results/URBCLIM_historical/t_/t_data_", min_max_avg, "_", ff_c[city_n], ".csv"))
    ...
    ggsave(paste0("results/URBCLIM_historical/figures/city_monthly_elasticities_lcz_pctg_", ff_c[city_n], "_", min_max_avg, ".png"), height = 4, width = 6, scale=1.4)
    
    write.csv(result, paste0("results/URBCLIM_historical/monthly_coefs/monthly_coefs_", ff_c[city_n], "_", min_max_avg, ".csv"))
