...
# WBGT max with point extraction + monthly panel method
...
download_wbgt_file <- function(city_provide, year, var = "max") {
...
estimate_wbgt_panel <- function(city_provide, wbgt_var = "max", years = 2008:2017) {
  ...
  saveRDS(panel_df, file.path(OUTPUT_DIR, paste0(tolower(gsub(" ", "_", city_provide)),
                                               "_WBGTmax_monthly_panel_data.rds")))
...
run_all_cities_wbgt <- function(cities, wbgt_var = "max", years = 2008:2017) {
  all_results <- list()
