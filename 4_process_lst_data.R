...
# Output file:
outname <- paste0(stub0, "data_provide_tmax_lst_monthly_cities.rds")
...
for(min_max_avg_sel in c("mean", "max", "min")){
  ...
  ff = list.files(pattern=paste0("LST_year_daily_", min_max_avg, "_"), path="H:/ECIP/Falchetta/Downloads/compressed_daily", full.name=T, recursive=T)
  ...
  nnn <- dplyr::filter(nnn, original!="Bologna")
  nnn$distance <- NULL
  ...
  # hh = list.files(pattern="LST_year_daily_mean_", path="H:/ECIP/Falchetta/Downloads/compressed_daily", full.name=T, recursive=T)
  # ww = list.files(pattern="LST_year_daily_mean_", path="H:/ECIP/Falchetta/Downloads/compressed_daily", full.name=T, recursive=T)
  ...
  for(city_n in which(ff_c %in% out_ndvi_m$city_provide)){
    ...
    names(proj_city) <- paste0("LST_", 1:nlyr(proj_city))
    ...
    names(tas_out_m1_m) <- paste0("mean.LST_m", 1:12)
    ...
    tapply(out_ndvi_m_f$mean.LST_m6, out_ndvi_m_f$lcz, summary)
    
    ###
    
    out_ndvi_m_monthly = dplyr::select(out_ndvi_m_f, x, y, out_b, Cls, build_h , build_v , pop_dens , water , elevation , city , lcz, contains("mean.LST_m"))
    ...
    write.csv(out_ndvi_m_monthly_stats, paste0("results/URBCLIM_historical/lst_/lst_stats_", min_max_avg, "_", ff_c[city_n], ".csv"))
    
    #
    
    write.csv(out_ndvi_m_monthly %>% dplyr::select(city, lcz, variable, value), paste0("results/URBCLIM_historical/lst_/lst_data_", min_max_avg, "_", ff_c[city_n], ".csv"))
