...
## Output
pathout <- "ugs/after_points_100425_completedatabase.rds"
...
dfpoints <- merge(dfpointslcz, ghsnames %>% dplyr::filter(ID_HDC_G0 != 11632) %>% dplyr::mutate(CTR_MN_NM=countrycode::countrycode(CTR_MN_NM, "country.name", "iso2c")),
                  by.x = c("city", "country"),
