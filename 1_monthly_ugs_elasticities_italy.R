...
setwd(path_provide)
city_d = tolower(read.csv("cities_provide.csv")[c(64:73),]) %>% gsub(" ", "_", .)
...
min_max_avg = "max"
  ...
  ff_c = ff_c[ff_c %in% tools::toTitleCase(city_d_c)]
  ff = ff[grepl(paste(ff_c, collapse="|"), ff)]
  ...
  for(city_n in which(ff_c %in% out_ndvi_m$city_provide)){
   ...
   extent_r <- extent(stack(ff_filter[[1]]))
      
    proj_city <- rast(ff_filter)
    terra::ext(proj_city) <- as.vector(extent_r)
    ...
    out_ndvi_m_monthly$city = ff_c[city_n]
    
    outer[[city_n]] = out_ndvi_m_monthly
  ...
  saveRDS(outer, "F:/.shortcut-targets-by-id/13znqeVDfPULc4J_lQLbyW_Kmfa03o63F/3-Research/GIACOMO/systemic cooling poverty/glob_assess_energy_demand_reduction_potential/seme_data/2_ancillary_analysis_data.rds")
  ...
  m0 <- feols(value ~ out_b  | city + variable +  city^variable, data=outer)
  summary(m0, "twoway")
  
  m1 <- feols(value ~ out_b + build_h + pop_dens + water + elevation | city + variable +  city^variable, data=outer)
  summary(m1, "twoway")
  
  m2 <- feols(value ~ out_b + out_b^2 + build_h + pop_dens + water + elevation | city + variable +  city^variable, data=outer)
  summary(m2, "twoway")
  
  outer$out_b_bins <- cut(outer$out_b, quantile(outer$out_b, seq(0, 1, 0.2)))
  outer <- within(outer, out_b_bins <- relevel(out_b_bins, ref = 3))
  
  m3 <- feols(value ~ out_b_bins + build_h + pop_dens + water + elevation | city + variable +  city^variable, data=outer)
  summary(m3, "twoway")
  

  m0_a <- feols(value ~ out_b |  variable, data=outer)
  summary(m0_a, "twoway")
  
  
  m1_a <- feols(value ~ out_b + build_h + pop_dens + water + elevation |  variable, data=outer)
  summary(m1_a, "twoway")
  
  m2_a <- feols(value ~ out_b + out_b^2 + build_h + pop_dens + water + elevation | variable, data=outer)
  summary(m2_a, "twoway")
  
  outer$out_b_bins <- cut(outer$out_b, quantile(outer$out_b, seq(0, 1, 0.2)))
  outer <- within(outer, out_b_bins <- relevel(out_b_bins, ref = 3))
  
  m3_a <- feols(value ~ out_b_bins + build_h + pop_dens + water + elevation |variable, data=outer)
  summary(m3_a, "twoway")
  
  etable(m0_a, m1_a, m2_a, m3_a, m0, m1, m2, m3, vcov="cluster")
  etable(m0_a, m1_a, m2_a, m3_a, m0, m1, m2, m3, vcov="cluster", file = paste0("F:/.shortcut-targets-by-id/13znqeVDfPULc4J_lQLbyW_Kmfa03o63F/3-Research/GIACOMO/systemic cooling poverty/glob_assess_energy_demand_reduction_potential/seme_data/", "regtab1_ITA_across.tex"), tex = T, replace = T)
