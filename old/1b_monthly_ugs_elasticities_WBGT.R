...
# Output file:
outname <- paste0(stub0, "data_provide_WBGT_qv_monthly_cities.rds")
...
ff = list.files(pattern=paste0("WBGT_year_daily_", min_max_avg, "_"), path="H:/ECIP/Falchetta/Downloads/compressed_daily", full.name=T, recursive=T)
...
for(city_n in which(ff_c %in% out_ndvi_m$city)){
  ...
  names(proj_city) <- paste0("WBGT_", 1:nlyr(proj_city))
  ...
  WBGT_out <- exact_extract(proj_city, out_ndvi_m_f, "mean")
  WBGT_out <- bind_cols(WBGT_out)
  colnames(WBGT_out) <- time(proj_city)
  
  ###
  
  colnames(WBGT_out) <- paste0("d_", lubridate::yday(colnames(WBGT_out)))
  
  nms <- unique(names(WBGT_out))
  
  # Average columns with the same name
  WBGT_out <- sapply(nms, function(x) rowMeans(WBGT_out[names(WBGT_out) %in% x], na.rm = T))
  WBGT_out <- as.data.frame(WBGT_out)
  
  WBGT_out <- WBGT_out[,1:365]
  
  colnames(WBGT_out) <- time(proj_city)[1:365]
  ...
  ## rowMedians
  tas_out_m1_m <- pblapply(1:12, function(X){rowCustoms(as.matrix(WBGT_out[,month(unique(as.Date(time(proj_city)[1:365])))==X]), na.rm=T)})
  tas_out_m1_m <- bind_cols(tas_out_m1_m)
  tas_out_m1_m <- tas_out_m1_m
  names(tas_out_m1_m) <- paste0("mean.WBGT_m", 1:12)
  ...
  tapply(out_ndvi_m_f$mean.WBGT_m6, out_ndvi_m_f$lcz, summary)
  
  ###
  
  out_ndvi_m_monthly = dplyr::select(out_ndvi_m_f, x, y, out_b, Cls, build_h , build_v , pop_dens , water , elevation , city , lcz, contains("mean.WBGT_m"))
  ...
  write.csv(out_ndvi_m_monthly_stats, paste0("results/wbgt_stats_", min_max_avg, "_", ff_c[city_n], ".csv"))
  
#
  
  write.csv(out_ndvi_m_monthly %>% dplyr::select(city, lcz, variable, value), paste0("results/wbgt_data_", min_max_avg, "_", ff_c[city_n], ".csv"))
  
  ##
  ##
  
  m1 <- feols(value ~  variable:out_b:lcz + elevation | variable + lcz, data=out_ndvi_m_monthly)
  summary(m1, "cluster")
  
  # + build_h + build_v + pop_dens + water + elevation
  
  plot_predictions(m1, condition = list("out_b", "variable"=c(6,7,8), "lcz"))
  ...
  result <- result %>% separate(name, c("month", "B", "lcz"), sep =":") %>% dplyr::filter(B=="out_b") %>% dplyr::select(-B)
  
  result$sig = result$`Pr(>|t|)`<0.05
  
  result$month <- gsub("variable", "", result$month)
  result$month <- factor(result$month, levels=c(1:12))
  
  result$lcz <- gsub("lcz", "", result$lcz)
  
  result <- merge(result, out_ndvi_m_monthly_m, by.x=c("month", "lcz"), by.y=c("variable", "lcz"))
  ...
  ggplot(result)+
    geom_point(aes(x=month, y=Estimate, group=1), colour="red")+
    geom_ribbon(aes(x=month, ymin=Estimate-1.96*`Std. Error`, ymax=Estimate+1.96*`Std. Error`, group=1), fill="pink",  alpha=0.25)+
    facet_wrap(vars(lcz), scales = "free_y")+
    geom_hline(yintercept = 0)+
    ggtitle("° change in monthly average WBGT from GVI marginal change")+
    ylab("Marginal change")
  
  ggplot(result)+
    geom_point(aes(x=month, y=Estimate_d, group=1), colour="red")+
    geom_ribbon(aes(x=month, ymin=Estimate_d-1.96*`Std. Error_d`, ymax=Estimate_d+1.96*`Std. Error_d`, group=1), fill="pink",  alpha=0.25)+
    facet_wrap(vars(lcz), scales = "free_y")+
    geom_hline(yintercept = 0)+
    ggtitle("% change in monthly average WBGT from GVI marginal change")+
    ylab("Marginal change")
  
  ggsave(paste0("results/city_monthly_elasticities_lcz_pctg_", ff_c[city_n], "_", min_max_avg, "_wbgt.png"), height = 4, width = 6, scale=1.4)
  
  write.csv(result, paste0("results/monthly_coefs_", ff_c[city_n], "_", min_max_avg, "_wbgt.csv"))
