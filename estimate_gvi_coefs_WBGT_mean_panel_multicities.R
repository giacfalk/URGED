# estimate_gvi_coefs_WBGT_panel_multicities.R
# WBGT mean with point extraction + monthly panel method
...
download_wbgt_file <- function(city_provide, year, var = "mean") {
  filename <- sprintf("WBGT_year_daily_%s_%d.nc", var, year)
  ...
  message("  [", year, "] Downloading WBGT ", var, "...")
...
estimate_wbgt_panel <- function(city_provide, wbgt_var = "mean", years = 2008:2017) {
  ...
  message("\n================================================================")
  message("  WBGT PANEL - Point extraction + Monthly panel")
  message("  PROVIDE city: ", city_provide)
  message("  GVI city: ", city_gvi)
  message("  WBGT var: ", wbgt_var)
  message("================================================================\n")
  ...
  gvi_agg <- gvi_all %>%
    ...
    group_by(x, y) %>%
    summarise(out_b = mean(out_b, na.rm = TRUE), .groups = "drop")
  ...
  # checking/downloading WBGT
  message("\n[2] Checking WBGT data...")
  city_folder <- gsub(" ", "_", city_provide)
  nc_files <- file.path(DATA_DIR, city_folder, sprintf("WBGT_year_daily_%s_%d.nc", wbgt_var, years))
  ...
  if (length(missing_years) > 0) {
    ...
    tryCatch({
      nc_files <- sapply(years, function(y) download_wbgt_file(city_provide, y, wbgt_var))
    }, error = function(e) {
      warning("Failed to download WBGT for ", city_provide, ": ", e$message)
      return(NULL)
    })
  }
  
  if (!all(file.exists(nc_files))) {
    warning("Missing WBGT files for ", city_provide)
    return(NULL)
  }
  
  # looading rasters
  message("\n[3] Loading WBGT rasters...")
  ...
  dx <- abs(mean(diff(x_vals)))
  dy <- abs(mean(diff(y_vals)))
  xmin <- min(x_vals) - dx/2; xmax <- max(x_vals) + dx/2
  ymin <- min(y_vals) - dy/2; ymax <- max(y_vals) + dy/2
  
  wbgt_list <- lapply(nc_files, function(f) {
    r <- rast(f)
  ...
  message("  Total days: ", nlyr(wbgt_stack))
  ...
  # extracting WBGT (point extraction)
  message("\n[5] Extracting WBGT at points...")
  pts_wgs <- st_as_sf(gvi_agg, coords = c("x", "y"), crs = 4326, remove = FALSE)
  pts_proj <- st_transform(pts_wgs, epsg_city)
  wbgt_extract <- terra::extract(wbgt_stack, vect(pts_proj))
  wbgt_mat <- as.matrix(wbgt_extract[, -1])
  
  # WBGT should already be in Celsius, but checking for Kelvin
  if (any(wbgt_mat > 200, na.rm = TRUE)) {
    message("  Converting Kelvin to Celsius...")
    wbgt_mat <- wbgt_mat - 273.15
  }
  message("  WBGT range: ", round(min(wbgt_mat, na.rm=TRUE), 1), " - ",
          round(max(wbgt_mat, na.rm=TRUE), 1), " C")
  ...
  dates <- dates[1:min(length(dates), ncol(wbgt_mat))]
  
  ym_vec <- format(dates, "%Y-%m")
  ym_list <- split(1:length(dates), ym_vec)
  
  wbgt_monthly <- sapply(ym_list, function(idx) {
    rowMeans(wbgt_mat[, idx, drop = FALSE], na.rm = TRUE)
  })
  
  n_points <- nrow(gvi_agg)
  n_months <- ncol(wbgt_monthly)
  ym_names <- colnames(wbgt_monthly)
  
  panel_df <- data.frame(
    point_id = rep(1:n_points, n_months),
    ym = rep(ym_names, each = n_points),
    wbgt = as.vector(wbgt_monthly),
  ...
  panel_df <- panel_df %>% filter(!is.na(wbgt) & !is.infinite(wbgt))
  
  saveRDS(panel_df, file.path(OUTPUT_DIR, paste0(tolower(gsub(" ", "_", city_provide)),
                                                   "_WBGTmean_monthly_panel_data.rds")))
  ...
  for (l in 1:9) {
    ...
    tryCatch({
      # model: wbgt ~ out_b:factor(month) + covariates | month + year
      m <- feols(wbgt ~ out_b:factor(month) + build_h + build_v + pop_dens + water + elevation | month + year,
                 data = df_lcz)
...
run_all_cities_wbgt <- function(cities, wbgt_var = "mean", years = 2008:2017) {
  all_results <- list()
  
  for (city in cities) {
    message("\n\n========== Processing: ", city, " ==========\n")
    tryCatch({
      res <- estimate_wbgt_panel(city, wbgt_var, years)
