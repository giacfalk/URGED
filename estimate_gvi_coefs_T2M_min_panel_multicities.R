...
# T2M min with point extraction + monthly panel method
...
download_T2M_file <- function(city_provide, year, var = "min") {
...
estimate_T2M_panel <- function(city_provide, T2M_var = "min", years = 2008:2017) {
  ...
  saveRDS(panel_df, file.path(OUTPUT_DIR, paste0(tolower(gsub(" ", "_", city_provide)),
                                                   "_T2Mmin_monthly_panel_data.rds")))
...
run_all_cities_T2M <- function(cities, T2M_var = "min", years = 2008:2017) {
  all_results <- list()
