...
# T2M mean with point extraction + monthly panel method
...
download_T2M_file <- function(city_provide, year, var = "mean") {
...
estimate_T2M_panel <- function(city_provide, T2M_var = "mean", years = 2008:2017) {
  ...
  panel_df <- panel_df %>% filter(!is.na(T2M) & !is.infinite(T2M))
    ...
    saveRDS(panel_df, file.path(OUTPUT_DIR, paste0(tolower(gsub(" ", "_", city_provide)),
                                               "_T2Mmean_monthly_panel_data.rds")))
...
run_all_cities_T2M <- function(cities, T2M_var = "mean", years = 2008:2017) {
  all_results <- list()
