...
for(min_max_avg_sel in c("mean")){
  ...
  for(city_n in which(ff_c %in% out_ndvi_m$city_provide)){
    ...
    tas_out <- exact_extract(proj_city, out_ndvi_m_f, "mean")
    tas_out <- bind_cols(tas_out)
    colnames(tas_out) <- time(proj_city)
    
    ###
    
    colnames(tas_out) <- paste0("d_", lubridate::yday(colnames(tas_out)))
    
    nms <- unique(names(tas_out))
    
    # Average columns with the same name
    tas_out <- sapply(nms, function(x) rowMeans(tas_out[names(tas_out) %in% x], na.rm = T))
    tas_out <- as.data.frame(tas_out)
    
    tas_out <- tas_out[,1:365]
    
    colnames(tas_out) <- time(proj_city)[1:365]
    
    ###
    
    out_ndvi_m_f = st_transform(out_ndvi_m_f, 4326)
    
    names(tas_out) <- paste0("mean.tas_", names(tas_out))
    
    tas_out_m1_m <- tas_out
    ...
    out_ndvi_m_monthly = dplyr::select(out_ndvi_m_f, x, y, out_b, Cls, build_h , build_v , pop_dens , water , elevation , city , lcz, contains("mean.tas_"))
