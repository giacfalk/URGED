...
for(min_max_avg_sel in c("mean", "min", "max")){
  ...
  for(city_n in which(ff_c %in% out_ndvi_m$city_provide)){
    ...
    write_rds(out_ndvi_m_monthly, paste0("results/regression_data/monthly_data_", ff_c[city_n], "_", min_max_avg, "_wbgt.rds"))
    ...
    saveRDS(broom::tidy(m1), file = paste0("results/URBCLIM_historical/regressions_/city_monthly_regression_", ff_c[city_n], "_", min_max_avg, "_wbgt_r1.rds")) # Export as .tex
    ...
    write.csv(result, paste0("results/URBCLIM_historical/monthly_coefs/monthly_coefs_", ff_c[city_n], "_", min_max_avg, "_wbgt_r1.csv"))
